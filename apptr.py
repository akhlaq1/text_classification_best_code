...
df = pd.read_csv(str(os.path.abspath(os.getcwd())).replace('\\', '/') + "/data_tr/" + 'trmails.csv')
print(df.head(10))
...
vectors = CountVectorizer(analyzer=process_text)
messages_bow = vectors.fit_transform(df['text'])
...
with open('classifier_vector.pkl', 'wb') as fid:
    cPickle.dump((vectors,classifier), fid)  
...
str1 = str(os.path.abspath(os.getcwd())).replace('\\', '/') + "/log_tr/" + dosya_sonuc
